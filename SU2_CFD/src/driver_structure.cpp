...
void CDriver::Solver_Preprocessing(CSolver ****solver_container, CGeometry ***geometry,
  ...
  if (model_split_hybrid) {
    ...
    CHybridForcingAbstractBase* forcing = new CHybridForcingTG0(geometry[val_iInst][MESH_0], config);
  ...
  for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++) {
    ...
    if (ns) {
      if (compressible) {
        ...
        if (model_split_hybrid) solver_container[val_iInst][iMGlevel][FLOW_SOL]->AddHybridMediator(hybrid_mediator);
      }
      if (incompressible) {
  ...
  if (model_split_hybrid) {
    unsigned long iPoint;
    for(iPoint=0; iPoint<geometry[val_iInst][0]->GetnPoint();iPoint++) {
      hybrid_mediator->ComputeResolutionAdequacy(geometry[val_iInst][0],solver_container[val_iInst][0],iPoint);
      const su2double rk = solver_container[val_iInst][0][FLOW_SOL]->node[iPoint]->GetResolutionAdequacy();
      solver_container[val_iInst][0][FLOW_SOL]->average_node[iPoint]->SetResolutionAdequacy(rk);
    }
  }
  ...
  if ((euler) || (ns)) {
    ...
    if (compressible) {
      if (model_split_hybrid) {
        ...
        numerics_container[val_iInst][MESH_0][FLOW_SOL][VISC_TERM] = new CAvgGrad_Hybrid(nDim, nVar_Flow, true, config);
        for (iMGlevel = 1; iMGlevel <= config->GetnMGLevels(); iMGlevel++)
          numerics_container[val_iInst][iMGlevel][FLOW_SOL][VISC_TERM] = new CAvgGrad_Hybrid(nDim, nVar_Flow, false, config);

        /*--- Definition of the boundary condition method ---*/
        for (iMGlevel = 0; iMGlevel <= config->GetnMGLevels(); iMGlevel++)
          numerics_container[val_iInst][iMGlevel][FLOW_SOL][VISC_BOUND_TERM] = new CAvgGrad_Hybrid(nDim, nVar_Flow, false, config);
