...
void COutput::SetConvHistory_Body(ofstream *ConvHist_file,
  ...
  if (rank == MASTER_NODE) {
    ...
    adj_turb_resid[1000], hybrid_resid[1000],
    ...
    *Surface_Buffet_Metric = NULL;
    ...
    nVar_Trans = 0, nVar_Heat = 0, nVar_Hybrid = 0,
      ...
      case DISC_ADJ_EULER:          case DISC_ADJ_NAVIER_STOKES:          case DISC_ADJ_RANS:
        ...
        if (dynamic_hybrid) {
          for (iVar = 0; iVar < nVar_Hybrid; iVar++)
            residual_hybrid[iVar] = solver_container[val_iZone][val_iInst][FinestMesh][HYBRID_SOL]->GetRes_RMS(iVar);
        }
